...
| 模块 | 应负责 | 不应负责 |
| --- | --- | --- |
| `clawkit-tools` | 工具契约、内置工具、MCP adapter、安全拦截 | Agent loop、终端 UI |
...
- `tools` 和 `memory` 是基础模块，不依赖 engine/cli/im。
- `engine` 依赖稳定的 Provider、Tool、Context、Memory、Observability 契约。
- `cli` 和 `im` 是入口适配层，不拥有 Agent 核心状态机。
- 新能力放入能表达职责的最窄模块；组合根可以装配具体实现，但不能承载业务逻辑。

## 当前实现事实

项目已经具备可运行的多模块原型和基础测试，但底层重构尚未完成：

- `AgentEngine` 仍是两千行以上的大类，包含循环、上下文、记忆、Skill、Session、Plan 和 SubAgent 逻辑。
- `ClawkitApp` 仍是一千行以上的大类，装配、REPL、命令、审批和 IM 生命周期尚未完全分离。
- `ToolCallExecutor`、`EphemeralContext`、`ApplicationBootstrap`、`ConsoleRenderer` 等组件已经出现，但旧路径仍存在。
- 普通 ReAct、Plan-and-Execute、SubAgent 和 internal tools 尚未全部进入同一工具执行链。
- 本地观测已有事件、trace 和 summary 骨架，但 `metrics.jsonl`、并发 run 隔离、Provider 作用域和独立测试仍不完整。

...
```text
1. 方案设计（Codex / 强模型）
   输出：完整可实现方案 + 单 PR 实施合同
   要求：代码锚点、修改范围、禁止事项、验收标准、测试命令、回滚方式
   ↓
2. 方案挑刺（DeepSeek / 交叉模型）
   只评 Blocking / Missing Tests / Scope Cuts / Conclusion
   禁止重设架构、扩展需求、提出非必要抽象
   ↓
3. 范围内实施（DeepSeek + Claude Code）
   按单 PR 合同实现；只改允许范围；超出范围先停止并报告
   ↓
4. 本地验证
   跑合同指定测试和 git diff --check；不通过不进入下一阶段
   ↓
5. 实现后 code review（Codex / 强模型）
   只查 bug、行为回归、漏测、边界破坏、无关改动
   禁止提出非必要重构或架构美化
   ↓
6. 收尾
...
- Codex 的方案要详细，但详细的是边界：任务范围、修改文件、禁止事项、验收标准和风险，不是堆抽象。
- DeepSeek 的方案评审只负责挑刺和减复杂度，重点找阻塞问题、测试缺口和可以砍掉或延后的设计。
- DeepSeek / Claude Code 实现时只吃一个小合同，不得顺手重构、扩大需求或修改无关文件。
- Codex 做实现后 review 时采用代码审查视角，只报 bug、回归、漏测、安全/权限绕过和模块边界破坏。
- 任一模型发现方案有问题时，应停止并报告，不自行扩大实现范围。

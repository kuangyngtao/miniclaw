...
public class ConsoleRenderer {
    ...
    public void renderMetrics(RunReader runReader, String runId) {
        ...
        try {
            ...
            if (runId != null) {
                var result = runReader.readMetrics(runId);
                m = result.value();
            } else {
                ...
                var result = runReader.readMetrics(runs.get(0).runId());
                m = result.value();
            }
            if (m == null) {
        ...
        try {
            ...
            if (!result.warnings().isEmpty()) {
                for (var w : result.warnings()) {
                    System.out.println(GRAY + "  ⚠ " + w.code() + " @ line " + w.lineNumber() + ": " + w.message() + RESET);
                }
            }
            System.out.println(GRAY + "  trace: " + runId + " (" + events.size() + " events)" + RESET);
            int shown = 0;
            for (var env : events) {
                String time = env.occurredAt() != null
                    ? java.time.LocalTime.from(env.occurredAt().atZone(java.time.ZoneId.systemDefault())).toString()
                    : "--:--:--";
                System.out.printf(GRAY + "  %03d %s %-30s turn=%s%n" + RESET,
                    env.sequence(), time, env.eventType(),
                    env.turnNumber() != null ? env.turnNumber().toString() : "-");
                if (++shown >= 20) {
                    System.out.println(GRAY + "  ... (" + (events.size() - shown) + " more events)" + RESET);
                    break;
                }
            }
            System.out.println();

...
public class LadderedCompactor implements ContextManager {
    ...
    /**
     * @deprecated 使用 LadderedCompactor(Summarizer, ContextBudgetAnalyzer) 替代
     */
    @Deprecated
    public LadderedCompactor(Summarizer summarizer, Tokenizer tokenizer) {
        this.summarizer = summarizer;
        this.tokenizer = tokenizer;
        this.budgetAnalyzer = new ContextBudgetAnalyzer(tokenizer, ContextBudgetPolicy.of(128_000));
    }

    public LadderedCompactor(Summarizer summarizer, ContextBudgetAnalyzer budgetAnalyzer) {
        this.summarizer = summarizer;
        this.budgetAnalyzer = budgetAnalyzer;
        this.tokenizer = budgetAnalyzer != null ? null : null; // 不直接存储，通过 analyzer 使用
    }

    // 获取 tokenizer（兼容旧路径）
    private Tokenizer tokenizer() {
        return tokenizer != null ? tokenizer : new CharFallbackTokenizer();
    }
    ...
    @Override
    public List<Message> applyAlwaysOnRules(List<Message> messages) {
        List<Message> result = new ArrayList<>(messages);
        var toolNameIndex = buildToolNameIndex(result);
        var boundary = findRecentTurnsBoundary(result);
        return applyAlwaysOnRulesImpl(result, boundary, toolNameIndex);
    }
    ...
    private Message applyCompressibleZoneAlwaysOn(Message msg, Map<String, String> toolNameIndex) {
        // Phase 4: TOOL 掩码已由 MessageMasker（始终运行）处理，
        // 此处只保留 ASSISTANT(tool_calls) 逻辑链保留

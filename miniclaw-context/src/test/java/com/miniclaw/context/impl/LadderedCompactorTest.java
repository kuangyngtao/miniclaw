...
class LadderedCompactorTest {
    ...
    @Test
    void shouldMaskToolOutputInCompressibleZone() {
        ...
        // 所有 TOOL 消息应被掩码为 "[tool:glob output — N bytes]"
        long maskedCount = result.stream()
            .filter(m -> m.role() == Role.TOOL)
            .filter(m -> m.content() != null && m.content().matches("\\[tool:glob output — \\d+ bytes\\]"))
            .count();
        // 保护区 (最近 3 USER 轮) 的 TOOL 不会被掩码，但超出 1000 字符才掐头去尾（这里都很短）
        // 压缩区有 10-3=7 个 TOOL 被掩码，保护区 3 个 TOOL 保留原样
        assertThat(maskedCount).isGreaterThanOrEqualTo(7);
    ...
    // === TOOL 掩码 fallback: toolCallId 无匹配 ASSISTANT ===

    @Test
    void shouldMaskToolOutputWithUnknownToolName() {
        List<Message> messages = new ArrayList<>();
        messages.add(Message.system("You are an assistant."));
        // 直接放 TOOL 消息，没有对应的 ASSISTANT tool_calls
        for (int i = 0; i < 8; i++) {
            messages.add(Message.user("task " + i));
        ...
        // 压缩区 TOOL 应被掩码为 "[tool:unknown output — N bytes]"
        long unknownCount = result.stream()
            .filter(m -> m.role() == Role.TOOL)
            .filter(m -> m.content() != null && m.content().contains("[tool:unknown"))
            .count();
        assertThat(unknownCount).isGreaterThanOrEqualTo(5); // 压缩区 5 个
    }
